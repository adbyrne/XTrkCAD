...
- **Visual pre/post diffing (`-d visualdiff=1`)** — the regression suite's `REGRESSION START`/`END`
  ...
  off-screen). `scripts/regression-visual-diff.sh <before-ref> <after-ref> <demo.xtr>` builds two
  ...
  "Regression Test Demos" page, generated by `scripts/generate-regression-docs.py`.
