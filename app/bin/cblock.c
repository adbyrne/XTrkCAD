 ...
 *      They do not report occupancy. They do not have a name or events.
...
static char * getEditSegs( char *temp, track_p trk );
...
static char blockScriptUnocc[STR_LONG_SIZE];
static char blockScriptOcc[STR_LONG_SIZE];
...
static char blockEditScriptOcc[STR_LONG_SIZE];
static char blockEditScriptUnocc[STR_LONG_SIZE];
static char blockEditSegs[STR_LONG_SIZE];
static evtBlk_p blockEditEvt;
...
typedef struct blockData_t {
    ...
    char * name;
    evtBlk_p prodEvt;  // produced events
    BOOL_T IsHilite;
    ...
    char * state;
    btrackinfo_t trackList; // Note trackList expands - has to be last...
...
static struct {
	...
	char occupied[STR_LONG_SIZE];
	char unoccupied[STR_LONG_SIZE];
	char segments[STR_LONG_SIZE];
	FLOAT_T length;
	coOrd endPt[2];
} blockData;

typedef enum { NM, OC, UO, SE, LN, E0, E1 } blockDesc_e;
static descData_t blockDesc[] = {
/*NM*/	{ DESC_STRING, N_("Name"), &blockData.name, sizeof(blockData.name) },
/*UO*/  { DESC_STRING, N_("Unoccupied"), &blockData.unoccupied, sizeof(blockData.unoccupied) },
/*OC*/  { DESC_STRING, N_("Occupied"), &blockData.occupied, sizeof(blockData.occupied) },
/*SE*/  { DESC_STRING, N_("Segments"), &blockData.segments, sizeof(blockData.segments) },
...
{
	blockData_p xx = GetblockData(trk);
	const char * thename, *theoccupied, *theunoccupied;
	char *newName, *newOccupied, *newUnoccupied;
	BOOL_T changed, nChanged, ocChanged, uoChanged;
	size_t max_str;

	LOG( log_block, 1, ("*** UpdateBlock(): needUndoStart = %d\n",needUndoStart))
	if ( inx == -1 ) {
		nChanged = ocChanged = uoChanged = changed = FALSE;
		...
		theunoccupied = wStringGetValue( (wString_p)blockDesc[UO].control0 );
		if ( strcmp( theunoccupied, xx->prodEvt->eId[0].script ) != 0 ) {
			uoChanged = changed = TRUE;
			max_str = blockDesc[UO].max_string;
			if ( GetLayoutOpenLCBmode() && ! VerifyAsciiEvent( theunoccupied ) ) {
	    			NoticeMessage( MSG_LCC_FORMAT_ERROR, _("Ok"), NULL);
			} else if ( max_str && strlen( theunoccupied ) > max_str-1 ) {
				newUnoccupied = MyMalloc(max_str);
				strncpy(newUnoccupied, theunoccupied, max_str - 1);
				newUnoccupied[max_str-1] = '\0';
				NoticeMessage2(0, MSG_ENTERED_STRING_TRUNCATED,
						_("Ok"), NULL, max_str-1);
			} else newUnoccupied = MyStrdup(theunoccupied);
		}
		theoccupied = wStringGetValue( (wString_p)blockDesc[OC].control0 );
		if ( strcmp( theoccupied, xx->prodEvt->eId[1].script ) != 0 ) {
			ocChanged = changed = TRUE;
			max_str = blockDesc[OC].max_string;
			if ( GetLayoutOpenLCBmode() && ! VerifyAsciiEvent( theoccupied ) ) {
	    			NoticeMessage( MSG_LCC_FORMAT_ERROR, _("Ok"), NULL);
			} else if ( max_str && strlen( theoccupied ) > max_str-1 ) {
				newOccupied = MyMalloc(max_str);
				strncpy(newOccupied, theoccupied, max_str - 1);
				newOccupied[max_str-1] = '\0';
				NoticeMessage2(0, MSG_ENTERED_STRING_TRUNCATED,
						_("Ok"), NULL, max_str-1);
			} else newOccupied = MyStrdup(theoccupied);
		}
		if ( ! changed ) return;
		...
		if ( nChanged ) {
			...
			xx->name = newName;
			UpdateOwnerName( xx->prodEvt, newName );
		}
		if ( uoChanged ) {
			UpdateEvent( &xx->prodEvt->eId[0], NULL, newUnoccupied );
		}
		if ( ocChanged ) {
			UpdateEvent( &xx->prodEvt->eId[1], NULL, newOccupied );
		}
...
static void DescribeBlock( track_p trk, char * str, CSIZE_T len )
{
	...
	strcpy( blockData.occupied, xx->prodEvt->eId[1].script );
	strcpy( blockData.unoccupied, xx->prodEvt->eId[0].script );
	strcpy( blockData.segments, getEditSegs( buf, trk ) );
	blockData.length = 0;
	BOOL_T first = TRUE;
	...
	blockDesc[NM].mode =
	blockDesc[OC].mode =
	blockDesc[UO].mode = DESC_NOREDRAW;
	blockDesc[SE].mode = BO_READONLY;
...
static void blockDebug( track_p b_trk )
{
	...
	LOG( log_block, 1, ("*** blockDebug(): B%d(%p) name = \"%s\"\n",
			GetTrkIndex(b_trk), b_trk, xx->name))
	if ( xx->prodEvt )
		DisplayEvtBlk( "blockDebug", xx->prodEvt );
...
// called by DeleteTrack->UndoDelete->FreeTrack
static void DeleteBlock( track_p t )
{
    ...
    if ( xx->prodEvt ) DeleteEvtBlk( xx->prodEvt );
...
static BOOL_T WriteBlock( track_p t, FILE * f )
{
	...
	rc &= fprintf(f, "BLOCK %d \"%s\"\n",
		GetTrkIndex(t), blockName)>0;
	rc &= WriteEndPt( f, t, 0 );
	rc &= WriteEndPt( f, t, 1 );
	if ( xx->prodEvt ) {
		rc &= fprintf(f, "\tSCRIPT \"UNOCCUPIED\" \"%s\"\n", xx->prodEvt->eId[0].script )>0;
		rc &= fprintf(f, "\tSCRIPT \"OCCUPIED\" \"%s\"\n", xx->prodEvt->eId[1].script )>0;
	}
...
static BOOL_T ReadBlock( char * line )
{
	...
	if ( ! GetArgs( line+6, "dq", &index, &name ) ) {
		return FALSE;
	}
	...
	blockScriptUnocc[0] = '\0';
	blockScriptOcc[0] = '\0';
#if 0
	LOG( log_block, 1, ("*** ReadBlock: index %d name %s\n", index, blockName))
#endif
	...
	while ( (cp = GetNextLine()) != NULL ) {
		...
		if ( strncmp( cp, "SCRIPT", 6 ) == 0 ) {
			if ( ! GetArgs( cp+6, "qq", &bufName, &bufScript ) ) return FALSE;
			if ( strncmp( bufName, "UNOCCUPIED", 10 ) == 0 ) {
				strncpy( blockScriptUnocc, bufScript, sizeof(blockScriptUnocc) );
				blockScriptUnocc[sizeof(blockScriptUnocc)-1] = 0;
#if 0
				LOG( log_block, 1, ("*** ReadBlock: T%d(%p) unoccupied %s\n",
					trkindex, trk, blockScriptUnocc))
#endif
			}
			if ( strncmp( bufName, "OCCUPIED", 8 ) == 0 ) {
				strncpy( blockScriptOcc, bufScript, sizeof(blockScriptOcc) );
				blockScriptOcc[sizeof(blockScriptOcc)-1] = 0;
#if 0
				LOG( log_block, 1, ("*** ReadBlock: T%d(%p) occupied %s\n",
					trkindex, trk, blockScriptOcc))
#endif
			}
		}
...
#if 0
		LOG( log_block, 2, ("*** addSegs(): switch/block at T%03d from T%03d-%d epCnt %d conBlock %p, B%d\n",
			GetTrkIndex(here),GetTrkIndex(from),epFrom,epCnt,here->conBlock,
			here->conBlock?GetTrkIndex(here->conBlock):0))
#endif
...
static track_p makeBlock( void )
{
	...
	// SetTrkBits( blk, TB_HIDEDESC);

	blk->endCnt = tempEndPts_da.cnt;
	for ( ep = 0; ep < tempEndPts_da.cnt; ep++ ) {
		endPtP = &tempEndPts(ep);
		blk->endPt[ep].prevTrack = endPtP->prevTrack;
		blk->endPt[ep].trackEp = endPtP->trackEp;
		blk->endPt[ep].pos = endPtP->pos;
		blk->endPt[ep].angle = endPtP->angle;
		blk->endPt[ep].option = 0;
#if 0
	LOG( log_block, 1, ( "*** makeBlock: T%d-%d -- endPt T%d-%d\n",
		trk0?GetTrkIndex(blk):0,ep,
		GetTrkIndex(endPtP->prevTrack),endPtP->trackEp))
#endif
	}
	// blk-ep[0] connects with this segment at ep0
	getDispTrkEp( blk, 0, &epB0, &trk0, &ep0 );
	// blk-ep[1] connects with this segment at ep1
	getDispTrkEp( blk, 1, &epB1, &trk1, &ep1 );

#if 0
	LOG( log_block, 1, ( "*** makeBlock: T%d-%d -- 0(T%d-%d)-B%d-(T%d-%d)1 -- T%d-%d\n",
		trk0?GetTrkIndex(trk0):0,ep0,
		GetTrkIndex(blk->endPt[0].prevTrack),epB0,
		GetTrkIndex(blk),
		GetTrkIndex(blk->endPt[1].prevTrack),epB1,
		trk1?GetTrkIndex(trk1):0,ep1))
	LOG( log_block, 1, ( "*** makeBlock: T%d-%d --  Unocc \"%s\" Occ \"%s\"\n\n",
				trk0?GetTrkIndex(trk0):0,ep0,blockScriptUnocc,blockScriptOcc))
#endif

	xx = GetblockData( blk );
	if ( blockName[0] != 'D' ) { // change the name to the block index
		if ( blockName[0] == '\0' )
			sprintf( blockName,"B%03d", GetTrkIndex( blk ) );
       		xx->prodEvt = NewBlockEvt( blk,  blockName );

		if ( ! xx->prodEvt && blockScriptUnocc[0] != '\0' && blockScriptOcc[0] != '\0' ) {
			UpdateEvent( &xx->prodEvt->eId[0], NULL, blockScriptUnocc );
			UpdateEvent( &xx->prodEvt->eId[1], NULL, blockScriptOcc );
		}
	} else
	...
	blockLen = 0;
#if 0
	LOG( log_block, 1, ( "*** makeBlock: B%d --\"%s\"-- ", GetTrkIndex(blk),xx->name) )
#endif
	for ( iTrack = 0; iTrack < blockTrk_da.cnt; iTrack++ ) {
		...
		if ( tracklist(iTrack).t ) {
			tracklist(iTrack).t->conBlock = blk;
			blk->occupied += tracklist(iTrack).t->occupied;
		}
#if 0
	...
	SetBlockBoundingBox( blk );

	DisplayEvtBlk( "makeBlock:", xx->prodEvt );
...
//      blockDebug( b_trk );
    ...
    if ( ! blockCheckContigiousPath( FALSE ) ) {
        AbortProg( MSG_BLOCK_PATH_ERROR );
    } else {
        rc = 4;
    }
...
// The block does not have a name or event block
...
static track_p  createDynamicBlock( track_p trk, EPINX_T ep )
{
	...
	blockName[0] = 'D'; // Dynamic block gets temp name "D", will be changed to ""
	blockName[1] = '\0';
...
static void verifyOccupancy ( BOOL_T rpt, BOOL_T all )
{
    track_p b_trk, trk, trk0, trk1, trkT;
    blockData_p xx;
    EPINX_T segOcc, iTrack, ep0, ep1, epB0, epB1, epT;
    EPINX_T problems = 0, report = 1;
	...
	// verify end points - if block, ep points outside of the block
	// referenced trk[ep] points to block ep
	trk0 = b_trk->endPt[0].prevTrack;
	trkT = trk0->endPt[b_trk->endPt[0].trackEp].track;
	epT = trk0->endPt[b_trk->endPt[0].trackEp].trackEp;
	if ( trkT->conBlock && trkT->conBlock == b_trk ) {
		LOG(log_block, 1, ("verifyOccupancy: endpoint 0 points back to block \n"))
		problems++;
	}
	if ( trk0 != trkT->endPt[epT].track ) {
		LOG(log_block, 1, ("verifyOccupancy: endpoint 0 error \n"))
		problems++;
	}
	trk0 = b_trk->endPt[1].prevTrack;
	trkT = trk0->endPt[b_trk->endPt[1].trackEp].track;
	epT = trk0->endPt[b_trk->endPt[1].trackEp].trackEp;
	if ( trkT->conBlock && trkT->conBlock == b_trk ) {
		LOG(log_block, 1, ("verifyOccupancy: endpoint 1 points back to block \n"))
		problems++;
	}
	if ( trk0 != trkT->endPt[epT].track ) {
		LOG(log_block, 1, ("verifyOccupancy: endpoint 1 error \n"))
		problems++;
	}
    }
    if ( problems ) {
...
static track_p FindBlockByName( char *name )
{
	...
	while ( a_trk ) {
		xx = GetblockData(a_trk);
		if ( xx->name && strcmp(xx->name, name) == 0 ) {
			return a_trk;
		}
		a_trk = xx->next_block;
	}
...
static void NewBlock( track_p sel_trk )
{
	track_p blk = NULL;

	LOG( log_block, 1, ("*** NewBlock()\n"))
	if ( ! sel_trk )
		return;

	LOG( log_block, 1, ("*** NewBlock( T%03d )\n", GetTrkIndex(sel_trk)))

	// Look for sel_trk in existing block
	blockEditName[0] = 0;
	while ( TrackIterate( &blk ) ) {
		if ( GetTrkType(blk) == T_BLOCK && TrackInBlock(sel_trk,blk) ) {
			EditBlock ( blk );
			return;
		}
	}
	...
	if ( ! IsTrack( sel_trk ) ) {
		LOG( log_block, 1, ("*** NewBlock( %s is not trk type )\n",
				GetTrkTypeName(sel_trk)))
		NoticeMessage( _("Please select a track segment"), _("Ok"), NULL);
		return;
	}

	if ( GetTrkEndPtCnt(sel_trk) != 2 ) {
		LOG( log_block, 1, ("*** NewBlock( turnout )\n"))
		NoticeMessage( _("Please select a track segment. Not a turnout."),
			_("Ok"), NULL);
		return;
	}
	...
	LOG( log_block, 1, ("*** NewBlock(): %d tracks in block\n",blockTrk_da.cnt))
	blk = makeBlock();
...
static void BlockEditOk( void * junk )
{
    ...
    UpdateOwnerName( xx->prodEvt, blockEditName );
    UpdateEvent( &xx->prodEvt->eId[0], NULL, blockEditScriptUnocc );
    UpdateEvent( &xx->prodEvt->eId[1], NULL, blockEditScriptOcc );
    xx->AutoGenerated = FALSE;
    DisplayEvtBlk( "BlockEditOk:", xx->prodEvt );
//  blockDebug(trk);
    UndoEnd();
    wHide( blockEditW );
}
...
static char * getEditSegs( char *temp, track_p trk )
{
    blockData_p xx = GetblockData(trk);
    wIndex_t iTrack;
    BOOL_T needComma = TRUE;

    temp[0] = '\0';
    for ( iTrack = 0; iTrack < xx->numTracks ; iTrack++ ) {
        if ( (&(xx->trackList))[iTrack].t == NULL ) continue;
        sprintf(&temp[strlen(temp)],"%d",GetTrkIndex((&(xx->trackList))[iTrack].t));
	if ( iTrack +1 == xx->numTracks )
        	needComma = FALSE;
        if ( needComma ) strcat(temp,", ");
    }
    return  MyStrdup(temp);
}
...
static void EditBlock( track_p trk )
{
    ...
    char temp[STR_SHORT_SIZE], buf[STR_SHORT_SIZE];

    strncpy(blockEditName, xx->name, STR_SHORT_SIZE - 1);
    blockEditName[STR_SHORT_SIZE-1] = '\0';
    strncpy( blockEditScriptUnocc, xx->prodEvt->eId[0].script, sizeof(blockEditScriptUnocc) );
    strncpy( blockEditScriptOcc, xx->prodEvt->eId[1].script, sizeof(blockEditScriptOcc) );

    strcpy( blockEditSegs, getEditSegs( buf, trk ) );
    blockEditTrack = trk;
    if ( ! blockEditW ) {
...
{
    track_p trk = (track_p) data, next, trkT;
    blockData_p xx = GetblockData(trk);
    wIndex_t iTrack;
    BOOL_T needComma = FALSE;
    char temp[32];
    ...
    case CONTMGM_GET_TITLE:
        ...
        getDispTrkEp( trk, 0, &epB, &next, &nextEp );
	if ( next->conBlock == trk ) {
		next = trk->endPt[0].prevTrack->endPt[nextEp].track;
	}
	if ( next->conBlock && GetTrkEndPtCnt( next->conBlock ) == 2 )
            sprintf(temp,"(B%3d) T%03d-%d < ", GetTrkIndex( next->conBlock ), GetTrkIndex( next ), nextEp );
	else
            sprintf(temp,"T%03d-%d < ", GetTrkIndex( next ), nextEp );
        strcat(message,temp);
        for ( iTrack = 0; iTrack < xx->numTracks ; iTrack++ ) {
            if ( (&(xx->trackList))[iTrack].t == NULL ) continue;
        ...
        getDispTrkEp( trk, 1, &epB, &next, &nextEp );
	if ( next->conBlock == trk ) {
		next = trk->endPt[1].prevTrack->endPt[nextEp].track;
        }
	if ( next->conBlock && GetTrkEndPtCnt( next->conBlock ) == 2 ) 
            sprintf(temp," > T%3d-%d (B%03d)",GetTrkIndex( next ), nextEp, GetTrkIndex( next->conBlock ));
	else
            sprintf(temp," > T%03d-%d", GetTrkIndex( next ), nextEp);
        strcat(message,temp);
        break;
