...
void ProblemrepCreateW(void* ptr)
{
	if (!problemrepW) {
		...
		problemrepW = ParamCreateDialog(&problemrepPG, MakeWindowTitle(_("Data for Problem Report")), NULL, NULL,
			wHide, TRUE, NULL, F_TOP | F_CENTER | PD_F_ALT_CANCELLABEL, NULL);
	} else {
...
ProblemSaveLayout(void)
{
	...
	if (GetLayoutChanged()) {
		...
		rc = wNoticeEx( NT_WARNING, _("Trackplan has to be saved first. Do you want to do so now?"), 
			N_("Save Now..."),
			N_("Cancel"));
