...
from xtrkcad_mcp.models import SCALE_RATIOS, car_length_model_inches
...
def get_track_lengths(path: str) -> dict:
    ...
    car_len_in = car_length_model_inches(layout.scale)
    cars_per_ft = 12.0 / car_len_in  # cars per real foot of track
    ...
    for idx, ft in sorted(by_layer.items()):
        ...
        layer_breakdown[str(idx)] = {
            ...
            "car_capacity": int(ft * cars_per_ft),
        }
    ...
    return {
        ...
        "total_car_capacity": int(total_ft * cars_per_ft),
...
def get_curve_stats(path: str) -> dict:
    ...
    return {
        ...
        "min_radius_real_in": round(min_r, 2),
        "radius_distribution": buckets,
    }


# ---------------------------------------------------------------------------
# Tools — Operation Density (Joe Fugate methodology)
# ---------------------------------------------------------------------------
...
def get_operation_density(
    ...
    """Compute Operation Density statistics for a layout (Joe Fugate methodology).

    Based on Joe Fugate's Layout Design Assessment methodology (MRH Oct 2014).
    Track length is measured in real feet (model inches ÷ 12). Car capacity is
    total_real_ft × (12 / car_length_model_in).
    ...
    Args:
        ...
        layer_categories: Optional dict mapping layer index (as string) or layer
            ...
            Example: {"0": "mainline", "1": "staging"}.
            Uncategorized layers default to "mainline".
        car_length_model_in: Override car length in model inches. Defaults to
            a standard 40-ft freight car for the layout's scale.
        train_length_cars: Assumed train length in cars (default 6).

    Returns:
        Dict with room_area, total track by category, turnout counts, car
        capacity, and operating estimates (max cars, cars moved, trains).
    """
    layout = parse_file(_resolve_plan(path))
    scale = layout.scale

    if car_length_model_in is None:
        car_length_model_in = car_length_model_inches(scale)
    cars_per_real_ft = 12.0 / car_length_model_in

    # Build layer → category map
    # Accepts {"0": "mainline"} or {"Main": "mainline"}
    ...
    total_cars = total_ft * cars_per_real_ft

    # Operational capacity (cars that can actually be switched, excluding
    # through routes — mainline + connecting are pipeline, not car storage)
    pipeline_ft = cat_ft.get("mainline", 0.0) + cat_ft.get("connecting", 0.0)
    switchable_ft = total_ft - pipeline_ft
    max_cars = int(switchable_ft * cars_per_real_ft)

    # Cars moved per session ≈ total_ft / 4 (empirical, matches Fugate examples)
    cars_moved = int(total_ft / 4.0)
    trains = round(cars_moved / max(train_length_cars, 1), 1)

    # Room
    room_w_ft = layout.room_width / 12.0
    room_h_ft = layout.room_height / 12.0
    room_area = room_w_ft * room_h_ft
    ...
    for cat in sorted(cat_ft):
        ...
        categories_out[cat] = {
            "length_real_ft": round(ft, 1),
            "car_capacity": int(ft * cars_per_real_ft),
    ...
    return {
        ...
        "cars_per_real_ft": round(cars_per_real_ft, 2),
        "room": {
            "width_ft": round(room_w_ft, 1),
            "height_ft": round(room_h_ft, 1),
            "area_sqft": round(room_area, 0),
        },
        "totals": {
            "length_real_ft": round(total_ft, 1),
            "car_capacity": int(total_cars),
            "turnout_count": total_turnouts,
        },
        "by_category": categories_out,
        "operations": {
            "switchable_ft": round(switchable_ft, 1),
            "max_active_cars": max_cars,
        ...
        "note": (
            ...
            "cars_moved_per_session ≈ total_ft / 4. "
            "max_active_cars excludes mainline/connecting pipeline track. "
            "Provide layer_categories to split mainline vs branchline vs staging."
        ),
    }
