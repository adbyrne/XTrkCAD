        ...
        map_t * map )
{
	...
	while ( fgets( line, sizeof line, mapF ) != NULL ) {
		...
		if ( map->cnt+1 > map->max ) {
			map->max += 10;
			map->map = (map_element_t*)realloc( map->map,
			                                    map->max * sizeof *(map_element_t*)0 );
		}
		cp1 = strchr( line, '\t' );
		if ( cp1 == NULL ) {
	...
	while ( fgets( line, sizeof line, inF )  != NULL ) {
		...
		if ( strnicmp( line, "scale=", 6 ) == 0 ) {
			...
			if ( stricmp( scale, "N" ) == 0 ) {
				...
				ratio = 64.0;
			} else if ( stricmp(scale, "O") == 0 || stricmp(scale, "On3") == 0
			            || stricmp(scale, "On30") == 0 ) {
				ratio = 48.0;
				...
				ratio = 22.5;
			} else if ( stricmp(scale, "F") == 0 || stricmp(scale, "Fn2") == 0
			            || stricmp(scale, "Fn3") == 0 ) {
				ratio = 20.32;
			} else {
			...
			fprintf( outF,
			         "CARPART %s \"%s\t%s\t%s%s%s\t%s\t%s\t%s\t%s\" %d %d %0.3f %0.3f 0 0 %0.3f %0.3f %ld\n",
