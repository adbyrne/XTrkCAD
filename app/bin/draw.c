/** \file draw.c
 * @brief Main drawing window management: layout, zoom, pan, mouse, and init.
 *
 * Low-level drawing primitives live in drawprim.c.
 * Ruler rendering and border metrics live in drawruler.c.
 */
...
#include "drawruler.h"   // @brief for lborder, bborder, DrawRulerInit()

#define NUM_INFOCTL (4)

EXPORT wIndex_t panCmdInx;

// @brief Forward declarations for functions defined later in this file
...
/* @brief Update temp_surface after executing a command. */
EXPORT void TempRedraw(void) {

  static int cTR = 0;
  LOG(log_redraw, 2, ("TempRedraw: %d\n", cTR++));
  if (wDrawDoTempDraw == FALSE) {
    // @brief Windows does not yet support GTK TempDraw
    MainRedraw();
  } else {
...
/**
 * @brief Calculate position and size of background bitmap.
 *
 * @param drawP   Destination drawing area.
 * @param origX   x origin of drawing area.
 * @param origY   y origin of drawing area.
 * @param posX    OUT x position of bitmap.
 * @param posY    OUT y position of bitmap.
 * @param pWidth  OUT width of bitmap in destination coordinates.
...
/* @brief Redraw contents of the main window. */
...
/**
 * @brief Layout main window in response to Pan/Zoom.
 *
 * @param bRedraw    TRUE to trigger a full MainRedraw.
 * @param bNoBorder  TRUE to prevent mainD.orig from going negative.
 */
EXPORT void MainLayout(wBool_t bRedraw, wBool_t bNoBorder) {
  ...
  if (bRedraw && wDrawDoTempDraw) {
    ...
    // @brief Prevent recursion if curCommand calls MainLayout on Move/Drag
...
/**
 * @brief The wlib event handler for the main window.
 *
...
/**
 * @brief Constrain mainD.orig so the view does not scroll past the room edges.
 *
 * @param orig      IN/OUT view origin to constrain.
 * @param size      Current view size.
 * @param bNoBorder TRUE to allow scrolling to the canvas centre only.
 * @param bRound    TRUE to snap origin to the pixelBins grid.
 */
...
/**
 * @brief Initialize the menu for setting zoom factors.
 *
 * @param zoomM     Menu to which radio button is added.
 * @param zoomSubM  Second menu (or NULL).
 * @param ctxMenu1  Context menu 1 (or NULL).
 * @param panMenu   Pan popup menu (or NULL).
 */
...
/**
 * @brief Activate the radio button(s) corresponding to the current scale.
 * @param scale  Current drawing scale.
 */
...
/**
 * @brief Return the index of @p scale in zoomList, or -1 if not found.
 */
...
/**
 * @brief Return the nearest zoomList index for @p scale.
 * @param larger  TRUE to find the next larger value, FALSE for smaller.
 */
...
/**
 * @brief Apply a new drawing scale and refresh the display.
 *
...
/**
 * @brief User selected zoom in, via mouse wheel, button or pulldown.
 *
 * @param mode  NULL if zoom button was activated; non-NULL if via popup/wheel.
 */
...
/**
 * @brief Zoom to fit the room or current selection.
 *
 * @param mode  0 = fit room extents; 1 = fit selected tracks.
 */
...
/**
 * @brief User selected zoom out, via mouse wheel, button or pulldown.
 *
 * @param mode  NULL if zoom button was activated; non-NULL if via popup/wheel.
 */
...
/**
 * @brief Callback for user-selectable preset zoom menu items.
 * @param pScaleVP  Pointer to the DIST_T zoom value for this entry.
 */
static void DoZoom(void *pScaleVP) {
...
/**
 * @brief Position mainD based on panCenter.
 *
 * @param mode  0: constrain if constrainMain==1 or CTRL pressed.
...
/* @brief Is distance smaller than 10 pixels at 72 DPI / 0.1" at 1:1? */
...
/**
 * @brief Return the current position of the mouse pointer in pixel coordinates.
 *
 * @param x  OUT pointer pixel X.
 * @param y  OUT pointer pixel Y.
 */
...
/**
 * @brief Compute the minimum pan increment for the current scale and units.
 */
...
EXPORT void InitColor(void) {
  ...
  drawColorPreviewSelected = wDrawFindColor(wRGB(6, 6, 255));
  drawColorPreviewUnselected = wDrawFindColor(wRGB(255, 215, 6));
...
EXPORT void DrawInit(int initialZoom) {
  ...
  // @brief Compute ruler font metrics and border sizes
  DrawRulerInit(mainD.d);
