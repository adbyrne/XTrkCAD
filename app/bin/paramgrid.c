/** \file paramgrid.c
 * Handle all the dialog box creation for wlibs that support a layout manager.
 */

/*  XTrkCad - Model Railroad CAD
 *  Copyright (C) 2022 Dave Bullis, Martin Fischer
 *
...
/**
 * \brief Read the stored values for all parameter fields from the configuration 
 * file. 
 * 
 * \param pg Dialog definition 
 *
 */
	...
	wWinPix_t *retH)
{
	...
	{
		paramData_p pd = (group->paramPtr)+inx;
		if ((pd->option & PDO_DLGIGNORE) != 0)
			continue;
		if (pd->option & ( PDO_DLGNEWCOLUMN | PDO_DLGHORZ ))
			curColumn++;
		else 
			curRow++;

		pd->posColumn = curColumn;
		pd->posRow = curRow;
	}
	...
	// 	if ((pd->option & PDO_DLGIGNORE) != 0)
	// 		goto SkipControl;
	// 	LOG(log_paramLayout, 2, ("%2d: Col %dx%d..%dx%d Ctl %dx%d..%dx%d %s\n", inx, columnK.orig.x, columnK.orig.y, columnK.term.x, columnK.term.y, controlK.orig.x, controlK.orig.y, controlK.term.x, controlK.term.y, pd->nameStr))
	// 	if (pd->type == PD_MENUITEM)
	// 	{
	// 		proc(pd, helpStr, 0, 0);
	// 		continue;
	...
	// 	if ((pd->option & PDO_DLGIGNORE) == 0)
	// 	{
	// 		LOG(log_paramLayout, 2, ("    Col %dx%d..%dx%d Ctl %dx%d..%dx%d\n", columnK.orig.x, columnK.orig.y, columnK.term.x, columnK.term.y, controlK.orig.x, controlK.orig.y, controlK.term.x, controlK.term.y))
	// 	}
	// 	if (windowK.term.x < columnK.term.x)
	// 		windowK.term.x = columnK.term.x;
	// 	if (windowK.term.y < columnK.term.y)
		...
		paramChangeProc changeProc )
{
	...
	if ( group->cancelProc ) {
//		wButtonForDialog( group->win, NULL, cancelLabel, BB_DEFAULT, ParamButtonCancel, BRESULT_CANCEL );
//		group->cancelB = wButtonNewInDialog( group->win, 0, 0, NULL, cancelLabel, BB_CANCEL, 0, ParamButtonCancel, group );
	}
	if ( needHelpButton ) {
		sprintf( helpStr, "cmd%s", group->nameStr );
		helpStr[3] = toupper((unsigned char)helpStr[3]);
//		wButtonForDialog( group->win, NULL, _("Help"), BB_HELP, (wButtonCallBack_p)wHelp, BRESULT_HELP );
//		group->helpB = wButtonNewInDialog( group->win, 0, 0, NULL, _("Help"), BB_HELP, 0, (wButtonCallBack_p)wHelp, MyStrdup(helpStr) );
	}
	if ( okLabel && okProc ) {
		sprintf( helpStr, "%s-ok", group->nameStr );
//		wButtonForDialog( group->win ,helpStr, okLabel, BB_DEFAULT, ParamButtonOk, BRESULT_OK );
//		group->okB = wButtonNewInDialog( group->win, 0, 0, helpStr, okLabel, BB_DEFAULT, 0, ParamButtonOk, group );
	}
