 ...
 ****************************************************************************
 *
 *   Phil Cameron -- Block Signaling
 *
 *   Signals are placed at each end of every block. Signals between blocks
 *   manage entry into next block. Signals entering a turnout is a little more
 *   complex. A signal entering the points end of the turnout has one signal
 *   head for each turnout position. The "normal" position is the top signal
 *   head. Otherwise, the signal has a single head. When a turnout is on a
 *   route between two blocks, any endpoints that are not on a route need
 *   a signal as well.
 *
 *   When there are multiple heads on a signal mast the one that represents
 *   the selected turnout position displays the active aspect. The remaining
 *   heads display the most restrictive aspect (usually Red).
 *
 *   The active signal head can show one aspect at a time from a set of
 *   asapecs. The aspects are organized into an event block.
 *
 *   The signals have, by default, a heads that have a multi-color
 *   indicator. This permits the following aspects and indications:
 *     off    - turnout does not currently connect blocks.
 *     red    - stop. This can be displayed for a number of conditions
 *              an open turnout or occupied block/
 *     yellow - the signal beyond this signal is indicating red.
 *            - this is also set when a turnout does not lead to another
 *              block.
 *     green  - Turnout is aligned to enter a block and the following signal
 *              is either green or yellow.
 *
 *   Track that is not in a block and turnouts that do not on routes that
 *   connect blocks do not have signals.
 *
 ****************************************************************************/
...
typedef struct signalData_t {
    ...
    ANGLE_T angle;
    char * name;
    track_p refBlock; // signal references this block at
    EPINX_T refBlockEp; // this ep
    track_p refTrack; // signal references this track at
    EPINX_T refEp;    // this endpoint
    evtBlk_p prodSigEvt;
    wIndex_t numHeads;
    int currHead;	// currently active head
    int currAspect;	// current aspect
    BOOL_T IsHilite;
    BOOL_T AutoGenerated; // signal was auto generated and not created, edited or saved
    wIndex_t numAspects;
    signalAspect_t aspectList; // Must be last
...
wDrawColor aspects[4];

#define signal_SF (3.0)

// Draw a black signal mast with the indicated number of heads
// The aspects shown ( black, red, yellow, green) are fixed (at present).
static void DDrawSignal(drawCmd_p d, coOrd orig, ANGLE_T angle, 
                        wIndex_t numHeads, int head, int aspect, DIST_T scaleRatio,
                        wDrawColor color )
{
    coOrd p1, p2;
    ...
    DrawLine(d, p1, p2, 6, wDrawColorBlack);
    p1 = orig;
	...
	if ( numHeads - 1 - ihead != head )
		color = drawColorRed;
	else
		color = aspects[aspect];

        DrawFillCircle(d,p1,HEADR * signal_SF / scaleRatio,drawColorBlack);
        DrawFillCircle(d,p1,(HEADR-3) * signal_SF / scaleRatio,color);
        hoffset += HEADR*2;
    }
}

static void DrawSignal (track_p sig, drawCmd_p d, wDrawColor color )
{
    signalData_p xx;
    int asp = ASP_OFF;
    int head;

    if ( GetTrkType( sig ) != T_SIGNAL ) AbortProg( "Not a signal!" );

    xx = GetsignalData(sig);

    CalcThisSignal(xx->refTrack, xx->refEp, &head, &asp );

    DDrawSignal(d,xx->orig, xx->angle, xx->numHeads, head, asp,
		   GetScaleRatio(GetTrkScale(sig)), color);
}
...
EXPORT BOOL_T ResolveBlockSignal( track_p trk )
{
	//PHIL fill this in
}
                                     ...
                                     descUpd, BOOL_T needUndoStart )
{
    ...
    case -1:
        ...
        if (nChanged) {
            MyFree(xx->name);
            xx->name = newName;
            UpdateOwnerName( xx->prodSigEvt, newName );
        }
...
static void DeleteSignal ( track_p trk )
{
    ...
    if ( xx->prodSigEvt ) DeleteEvtBlk( xx->prodSigEvt );
    xx->prodSigEvt = NULL;
...
static BOOL_T WriteSignal ( track_p t, FILE * f )
{
    ...
    trk = xx->refBlock?xx->refBlock:xx->refTrack;

    rc &= fprintf(f, "SIGNAL %d %u %s %d %0.6f %0.6f %0.6f %d \"%s\" %d %d\n",
                  GetTrkIndex(t), GetTrkLayer(t), GetTrkScaleName(t), 
                  GetTrkVisible(t), xx->orig.x, xx->orig.y, xx->angle, 
                  xx->numHeads, signalName,
		  trk?GetTrkIndex(trk):0,
		  xx->refBlockEp?xx->refBlockEp:xx->refEp)>0;
    for (ia = 0; ia < xx->numAspects; ia++) {
...
static BOOL_T ReadSignal ( char * line )
{
    ...
    sig = NewTrack(index, T_SIGNAL, 0, sizeof(signalData_t)+(sizeof(signalAspect_t)*(signalAspect_da.cnt-1))+1);
    ...
    xx->orig = orig;
    xx->angle = angle;
    xx->refTrack = FindTrack(trkindex);
    xx->refEp = refEp;
    if ( GetTrkType(xx->refTrack) == T_BLOCK ) {
        xx->refBlock = xx->refTrack;
        xx->refBlockEp = refEp;
        trk0 = xx->refTrack->endPt[refEp].prevTrack;
        ep0 = xx->refTrack->endPt[refEp].trackEp;
        xx->refTrack = trk0->endPt[ep0].track; // turnout/trk seg
        xx->refEp = trk0->endPt[ep0].trackEp; // turnout's ep
    }
    xx->numAspects = signalAspect_da.cnt;
    if ( xx->numAspects > 0 ) {
        xx->prodSigEvt = NewSignalEvt( sig, name, xx->numAspects );
        xx->prodSigEvt->numEvt = xx->prodSigEvt->state = xx->numAspects;
    }
    for (ia = 0; ia < xx->numAspects; ia++) {
        (&(xx->aspectList))[ia].aspectName = signalAspect(ia).aspectName;
        (&(xx->aspectList))[ia].aspectScript = signalAspect(ia).aspectScript;
	if ( xx->prodSigEvt )
            UpdateEvent( &xx->prodSigEvt->eId[0], signalAspect(ia).aspectName, signalAspect(ia).aspectScript );
    }
//    Translate ( &xx->orig, xx->orig, xx->angle + 180.0, 1.5*trackGauge );
    ComputeSignalBoundingBox(sig);
    DisplayEvtBlk( "SIG", xx->prodSigEvt );
...
static void SignalEditOk ( void * junk )
{
    ...
    if (signalCreate_P) {
        ...
        xx->prodSigEvt = NewSignalEvt( trk, signalEditName, 0 );
    } else {
    ...
    if ( xx->name == NULL || strncmp (xx->name, signalEditName, STR_SHORT_SIZE) != 0) {
        if (xx->name != NULL ) MyFree(xx->name);
        xx->name = MyStrdup(signalEditName);
	UpdateOwnerName( xx->prodSigEvt, signalEditName );
    }
    xx->numAspects = signalAspect_da.cnt;
    xx->currAspect = xx->prodSigEvt->state;
    for (ia = 0; ia < xx->numAspects; ia++) {
...
{
    track_p trk, trk0, sig;
    EPINX_T ep, ep0, minEp = 0;
    DIST_T minDist, d;
    int ia, head, aspect;
    ...
    signalCreate_P = TRUE;
    if ((trk = OnTrack(&orig, FALSE, FALSE)) == NULL ||
	    ( GetTrkType( trk ) != T_BLOCK && GetTrkType( trk ) != T_TURNOUT )) {
	NoticeMessage( _("Signal must be on block or turnout"), _("Cancel"), NULL );
	return;
    }
    trk = trk->conBlock?trk->conBlock:trk;

    UndoStart( _("Create Signal"), "Create Signal");
    sig = NewTrack(0, T_SIGNAL, 0, sizeof(signalData_t)+(sizeof(signalAspect_t)*3)+1);
    xx = GetsignalData ( sig );

    // Find nearest endpoint
    minDist = FindDistance( GetTrkEndPos( trk, 0 ), orig );
    for ( ep = 1; ep < GetTrkEndPtCnt( trk ); ep ++ ) {
	    if ( ( d = FindDistance( GetTrkEndPos( trk, ep ), orig ) ) < minDist ) {
		    minDist = d;
		    minEp = ep;
	    }
    }
    xx->numHeads = GetHeadCount( trk, minEp, &dwarf );
    sprintf( buf, "S%s%03d-%d", GetTrkType(trk)==T_BLOCK?"B":"T",GetTrkIndex( trk ), minEp );
    xx->name = MyStrdup(buf);
    xx->refTrack = trk;
    xx->refEp = minEp;
	if ( GetTrkType(trk) == T_BLOCK ) {
		xx->refBlock = trk;
		xx->refBlockEp = ep;
		trk0 = trk->endPt[ep].prevTrack;
		ep0 = trk->endPt[ep].trackEp;
		xx->refTrack = trk0->endPt[ep0].track; // turnout/trk seg
		xx->refEp = trk0->endPt[ep0].trackEp; // turnout's ep
	}
    xx->orig = GetTrkEndPos( trk, minEp );
    xx->angle = GetTrkEndAngle( trk, minEp );
    Translate ( &xx->orig, xx->orig, xx->angle + 90.0, 1.75*trackGauge );
    Translate ( &xx->orig, xx->orig, xx->angle + 180.0, 1.5*trackGauge );
    LOG( log_signal, 1, ("*** CreateNewSignal() T%d-%d orig %0.2f %0.2f angle %0.0f name %s\n",
            GetTrkIndex(trk),minEp, xx->orig.x, xx->orig.y, xx->angle, xx->name))

    xx->prodSigEvt = NewSignalEvt( sig, xx->name, 0 );
    CalcThisSignal( xx->refTrack, xx->refEp, &head, &aspect );
    xx->prodSigEvt->state = aspect;
    for (ia = 0; ia < xx->prodSigEvt->numEvt; ia++) {
        (&(xx->aspectList))[ia].aspectName = MyStrdup( xx->prodSigEvt->eId[ia].name );
        (&(xx->aspectList))[ia].aspectScript = MyStrdup( xx->prodSigEvt->eId[ia].script );
    }
    xx->numAspects = xx->prodSigEvt->numEvt;
...
// trk is the block/turnout the signal controls.
// ep is the end point.
static track_p makeSignal( track_p inTrk, EPINX_T inEp )
{
	track_p sig, trk0, trk = inTrk;
	EPINX_T ep0, ep = inEp;
	signalData_p xx;
	char buf[STR_SHORT_SIZE];
	int ia;
	...
	if ( GetTrkType(trk) == T_BLOCK ) {
		xx->refBlock = trk;
		xx->refBlockEp = ep;
		xx->orig = GetTrkEndPos( trk, ep );
		xx->angle = GetTrkEndAngle( trk, ep );
		trk0 = trk->endPt[ep].prevTrack;
		ep0 = trk->endPt[ep].trackEp;
		trk = trk0->endPt[ep0].track; // turnout/trk seg
		ep = trk0->endPt[ep0].trackEp; // turnout's ep
		if ( trk->conBlock && inTrk ) { //PHIL hack...
			ep0 = ep0==1?0:1;
			trk = trk0->endPt[ep0].track;
		}
	} else {
		xx->orig = GetTrkEndPos( trk, ep );
		xx->angle = GetTrkEndAngle( trk, ep );
	...
	// name SB - signal to sub-block, ST - signal into turnout, SD - dwarf into turnout
	if ( GetTrkType( inTrk ) == T_TURNOUT )
		sprintf( buf, "SD%03d-%d", GetTrkIndex( inTrk ), inEp );
	else if ( GetTrkEndPtCnt( trk ) <=2 )
		sprintf( buf, "SB%03d", GetTrkIndex( trk->conBlock ));
	else
		sprintf( buf, "ST%03d-%d", GetTrkIndex( trk ), ep );
	xx->name = MyStrdup(buf);

	CalcThisSignal( trk, ep, &xx->currHead, &xx->currAspect );
	xx->numHeads = GetHeadCount( trk, ep, &dwarf );
	LOG( log_signal, 1, ("*** makeSignal(S%03d) T%03d-%d %s  heads %d dwarf %d\n",
			GetTrkIndex(sig), GetTrkIndex(trk), ep, xx->name, xx->numHeads, dwarf))

	LOG( log_signal, 1, ("*** makeSignal(S%03d) orig %02.f %02.f angle %02.f\n",
			GetTrkIndex(sig), xx->orig, xx->angle))

	if ( dwarf ) { // dwarf
		Translate ( &xx->orig, xx->orig, xx->angle + 270.0, 1.65*trackGauge );
		Translate ( &xx->orig, xx->orig, xx->angle, 1.3*trackGauge );
		xx->angle += 180.0;
	} else {
		Translate ( &xx->orig, xx->orig, xx->angle + 90.0, 1.65*trackGauge );
		Translate ( &xx->orig, xx->orig, xx->angle + 180.0, 3.0*trackGauge );
	}

	LOG( log_signal, 1, ("*** makeSignal() T%d-%d orig %0.2f %0.2f angle %0.0f name %s\n",
		GetTrkIndex(trk),ep, xx->orig.x, xx->orig.y, xx->angle, xx->name))
	...
	xx->prodSigEvt->state = xx->prodSigEvt->numEvt;
	xx->numAspects = xx->prodSigEvt->numEvt;
	xx->prodSigEvt->state = xx->currAspect;
...
EXPORT void AddMissingSignals( void )
{
	track_p trk, sig;
	EPINX_T ep;
	BOOL_T hasRoute;

	LOG( log_signal, 1, ("*** AddMissingSignals(1) -- enter -- \n"))
	SetupControlElements();

	// Loop through all blocks. Create Signals for them.
	TRK_ITERATE(trk) {
		if ( IsTrackDeleted(trk) || GetTrkType(trk) != T_BLOCK ) continue;
		if ( GetTrkEndPtCnt( trk ) != 2 ) continue;

		LOG( log_signal, 1, ("*** AddMissingSignals(2) T%d  %s\n", GetTrkIndex(trk), GetTrkTypeName(trk)))
		sig = makeSignal( trk, 0 );
		sig = makeSignal( trk, 1 );
	}

	// Loop through all turnouts and set up needed dwarfs
	TRK_ITERATE(trk) {
		if ( ! IsTrack( trk ) ) continue;
		if ( GetTrkEndPtCnt( trk ) <= 2 ) continue;
		if ( GetTrkType(trk) != T_TURNOUT ) continue;

		hasRoute = FALSE;
		for ( ep = 0; ep < GetTrkEndPtCnt( trk ); ep ++ ) {
			if ( trk->endPt[ep].onRoute ) hasRoute = TRUE;
		}
		if ( ! hasRoute ) continue;

		for ( ep = 0; ep < GetTrkEndPtCnt( trk ); ep ++ ) {
			if ( trk->endPt[ep].onRoute ) continue;
			LOG( log_signal, 1, ("*** AddMissingSignals(3) T%d-%d make dwarf\n",
				GetTrkIndex(trk), ep))
			sig = makeSignal( trk, ep );
		}
	}
//	DoRedraw();
	LOG( log_signal, 1, ("*** AddMissingSignals(1) -- exit -- \n"))
}

// Delete all auto generated signals
EXPORT void DeleteAllBlockSignals( void )
{
	track_p sig;
	signalData_p xx;

	LOG( log_signal, 1, ("*** DeleteAllBlockSignals() -- enter\n"))

	// Loop through all track segs
	TRK_ITERATE(sig) {
		if ( IsTrackDeleted(sig) || GetTrkType(sig) == T_SIGNAL ) {
			LOG( log_signal, 1, ("*** DeleteAllBlockSignals() next block B%d\n",
				GetTrkIndex(sig)))
			xx = GetsignalData(sig);
			if ( ! xx->AutoGenerated ) continue;

//			DeleteSignal( sig );
			DeleteTrack (sig, FALSE);
		}
	}
}

// Calculate the aspect and head for each signal
EXPORT void CalculateAllSignals( BOOL_T changed_only )
{
	track_p sig;
	signalData_p xx;
	int head, aspect, prev_head, prev_aspect;
	BOOL_T changed = FALSE, changed_any = FALSE;
	char *asp[] = { "OFF", "RED", "YELLOW", "GREEN" };

	LOG( log_signal, 1, ("*** CalculateAllSignals() -- enter\n"))
	...
	TRK_ITERATE(sig) {
		...
		if ( ! IsTrackDeleted(sig) && GetTrkType(sig) == T_SIGNAL ) {
			...
			prev_head = xx->currHead;
			prev_aspect = xx->currAspect;
			CalcThisSignal( xx->refTrack, xx->refEp, &head, &aspect );
			if ( xx->currHead != head || xx->currAspect != aspect ) {
				xx->currHead = head;
				xx->currAspect = aspect;
				// produce new aspect event
				changed_any = changed = TRUE;
			}
			if ( changed && changed_only )
				LOG( log_signal, 1, ("*** CalculateAllSignals() for S%03d T%03d-%d   H %d %s"
						       "	CHANGED was head %d %s\n",
					GetTrkIndex(sig), GetTrkIndex(xx->refTrack), xx->refEp,
					xx->currHead, asp[xx->currAspect],
					prev_head, asp[prev_aspect]))
			else {
				LOG( log_signal, 1, ("*** CalculateAllSignals() for S%03d T%03d-%d   H %d %s",
					GetTrkIndex(sig), GetTrkIndex(xx->refTrack), xx->refEp,
					xx->currHead, asp[xx->currAspect]))
				if ( changed )
					LOG( log_signal, 1, (" CHANGED was head %d %s",
								prev_head, prev_aspect))
				LOG( log_signal, 1, ("\n"))
			}
		}
	}
...
static int SignalMgmProc ( int cmd, void * data )
{
    ...
    case CONTMGM_GET_TITLE:
        sprintf(message,"\t%s\t%s", xx->name,asp[xx->prodSigEvt->state]);
        break;
    }
    return FALSE;
}
...
EXPORT void InitTrkSignal ( void )
{
    ...
    aspects[0] = drawColorBlack;
    aspects[1] = drawColorRed;
    aspects[2] = drawColorGold;
    aspects[3] = drawColorGreen;
