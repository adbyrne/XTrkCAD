...
wControl_p
wBitmapCreate( wWin_p parent, wWinPix_t x, wWinPix_t y, char *helpStr,
               long options, const struct wIcon_t * iconP )
{
	wBitmap_p bt;
	GdkPixbuf *pixbuf;
	GtkWidget *image;
	...
	if (options&BO_USETEMPLATE) {
		...
		bt->reveal = (GtkRevealer *)wlibGetWidgetFromName(parent, helpStr, "reveal",
		                TRUE );
	} else {
		/*
		 * Depending on the platform, parent->widget->window might still be null
