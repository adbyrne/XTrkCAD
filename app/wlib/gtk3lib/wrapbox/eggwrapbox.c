...
/* GtkWidget      */
static GtkSizeRequestMode egg_wrap_box_get_request_mode (
        GtkWidget           *widget);
static void egg_wrap_box_get_preferred_width            (
        GtkWidget           *widget,
        gint                *minimum_size,
        gint                *natural_size);
static void egg_wrap_box_get_preferred_height           (
        GtkWidget           *widget,
        gint                *minimum_size,
        gint                *natural_size);
static void egg_wrap_box_get_preferred_height_for_width (
        GtkWidget           *box,
        gint                 width,
        gint                *minimum_height,
        gint                *natural_height);
static void egg_wrap_box_get_preferred_width_for_height (
        GtkWidget           *box,
        gint                 width,
        gint                *minimum_height,
        gint                *natural_height);
...
egg_wrap_box_class_init (EggWrapBoxClass *class)
{
	...
	widget_class->get_preferred_height_for_width =
	        egg_wrap_box_get_preferred_height_for_width;
	widget_class->get_preferred_width_for_height =
	        egg_wrap_box_get_preferred_width_for_height;
                             ...
                             gint            n_visible)
{
	...
	for (i = 0, list = cursor; (n_visible > 0 ? i < n_visible : TRUE)
	     && list; list = list->next) {
		EggWrapBoxChild *child = list->data;
		...
		if ((orientation == GTK_ORIENTATION_HORIZONTAL
		     && (child->packing & EGG_WRAP_BOX_H_EXPAND) != 0) ||
		    (orientation == GTK_ORIENTATION_VERTICAL
		     && (child->packing & EGG_WRAP_BOX_V_EXPAND) != 0)) {
			expand_children++;
		}
                ...
                gint             *nat_item_size)
{
	...
	for (list = cursor, i = 0; list && i < line_length; list = list->next) {
		...
		if (extra_pixels > 0
		    && ORIENTATION_SPREADING (box) == EGG_WRAP_BOX_SPREAD_EXPAND) {
			this_item_size++;
			extra_pixels--;
		}
                              ...
                              GtkRequestedSize *item_sizes)
{
	...
	for (list = priv->children, i = 0; list; list = list->next, i++) {
		...
		if (ORIENTATION_SPREADING (box) == EGG_WRAP_BOX_SPREAD_END
		    && i >= n_children - extra_items) {
			position += line_length - extra_items;
		}

		/* Round up the size of every column/row */
		item_sizes[position].minimum_size = MAX (item_sizes[position].minimum_size,
		                                    child_min);
		item_sizes[position].natural_size = MAX (item_sizes[position].natural_size,
		                                    child_nat);
	}
                           ...
                           gint              n_children)
{
	...
	try_line_size =
	        gather_aligned_item_requests (box, orientation, *line_length, item_spacing,
	                                      n_children, sizes);
	...
	for (try_length = *line_length + 1; try_line_size < avail_size; try_length++) {
		...
		try_line_size = gather_aligned_item_requests (box, orientation, try_length,
		                item_spacing,
                            ...
                            GtkAllocation *allocation)
{
	...
	EggWrapBoxPrivate  *priv = box->priv;
	gint                avail_size, avail_other_size, min_items, item_spacing,
	                    line_spacing;
	...
	if (priv->mode == EGG_WRAP_ALLOCATE_ALIGNED ||
		...
		if (line_length * item_spacing + (line_length + 1) * nat_item_size <=
		    avail_size) {
			line_length++;
		}
		...
		if (priv->mode == EGG_WRAP_ALLOCATE_HOMOGENEOUS) {
			...
			extra_pixels      = avail_size       - (line_length - 1) * item_spacing -
			                    item_size * line_length;
			...
			extra_line_pixels = gtk_distribute_natural_allocation (avail_other_size,
			                    n_lines, line_sizes);
		}
		...
		for (i = 0, line_count = 0, list = priv->children; list; list = list->next) {
			...
			allocate_child (box, child, item_offset, line_offset, this_item_size,
			                this_line_size);
		...
		/* Distribute space among lines naturally */
		sizes            = (GtkRequestedSize *)array->data;
		avail_other_size = gtk_distribute_natural_allocation (avail_other_size,
		                   array->len, sizes);
                ...
                gint                *natural_height)
{
	...
	if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
		...
		if (priv->mode == EGG_WRAP_ALLOCATE_ALIGNED ||
			...
			get_average_item_size (box, GTK_ORIENTATION_HORIZONTAL, &min_item_width,
			                       &nat_item_width);
			...
			/* After the above aproximation, check if we cant fit one more on the line */
			if (line_length * priv->horizontal_spacing + (line_length + 1) * nat_item_width
			    <= avail_size) {
				line_length++;
			}
			...
			/* Now we need the real item allocation size */
			item_size = (avail_size - (line_length - 1) * priv->horizontal_spacing) /
			            line_length;
			...
			{
				extra_pixels = (avail_size - (line_length - 1) * priv->horizontal_spacing) %
				               line_length;
			}
			...
			if (priv->mode == EGG_WRAP_ALLOCATE_HOMOGENEOUS) {
				...
				get_largest_size_for_opposing_orientation (box, GTK_ORIENTATION_HORIZONTAL,
				                item_size,
				                &min_item_height, &nat_item_height);
				...
				while (list != NULL) {
					list =
					        get_largest_size_for_line_in_opposing_orientation (box,
			...
			while (list != NULL) {
				list =
				        get_largest_size_for_free_line_in_opposing_orientation (box,
				                GTK_ORIENTATION_HORIZONTAL,
				                list, min_items, avail_size,
				                &min_line_height, &nat_line_height,
				                NULL, NULL);
		...
		GTK_WIDGET_GET_CLASS (widget)->get_preferred_height (widget, &min_height,
		                &nat_height);
	}
                ...
                gint                *natural_width)
{
	...
	if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
		...
		if (priv->mode == EGG_WRAP_ALLOCATE_ALIGNED ||
			...
			get_average_item_size (box, GTK_ORIENTATION_VERTICAL, &min_item_height,
			                       &nat_item_height);
			...
			/* After the above aproximation, check if we cant fit one more on the line */
			if (line_length * priv->vertical_spacing + (line_length + 1) * nat_item_height
			    <= avail_size) {
				line_length++;
			}
			...
			/* Now we need the real item allocation size */
			item_size = (avail_size - (line_length - 1) * priv->vertical_spacing) /
			            line_length;
			...
			if (priv->vertical_spreading != EGG_WRAP_BOX_SPREAD_EXPAND) {
				...
				/* Collect the extra pixels for expand children */
			{
				extra_pixels = (avail_size - (line_length - 1) * priv->vertical_spacing) %
				               line_length;
			}
			...
			if (priv->mode == EGG_WRAP_ALLOCATE_HOMOGENEOUS) {
				...
				/* Here we just use the largest height-for-width and
				 * add up the size accordingly */
				get_largest_size_for_opposing_orientation (box, GTK_ORIENTATION_VERTICAL,
				                item_size,
				                &min_item_width, &nat_item_width);
