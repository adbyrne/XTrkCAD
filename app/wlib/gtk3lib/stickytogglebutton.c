...
static gboolean sticky_toggle_button_button_press_event(GtkWidget *widget,
                GdkEventButton *event);
static gboolean sticky_toggle_button_button_release_event(GtkWidget *widget,
                GdkEventButton *event);
                ...
                GdkEventButton *event)
{
	...
	if (event->button == GDK_BUTTON_PRIMARY) {
		...
		if (!self->sticky_mode && !self->sticky_state) {
			self->long_press_timeout_id = g_timeout_add(500, long_press_timeout_callback,
			                              self);
		}
	}

	return GTK_WIDGET_CLASS(sticky_toggle_button_parent_class)->button_press_event(
	               widget, event);
}
                ...
                GdkEventButton *event)
{
	...
	return GTK_WIDGET_CLASS(
	               sticky_toggle_button_parent_class)->button_release_event(widget, event);
}
...
void sticky_toggle_button_set_sticky(StickyToggleButton *self,
                                     gboolean sticky_state)
{
...
void sticky_toggle_button_set_mode(StickyToggleButton* self,
                                   STICKY_TOGGLE_BUTTON_MODE mode)
{
	g_return_if_fail(IS_STICKY_TOGGLE_BUTTON(self));
	g_return_if_fail(mode == STICKY_TOGGLE_BUTTON_FIXED
	                 || mode == STICKY_TOGGLE_BUTTON_TEMP);
