    ...
    wDrawColor color)
{
    ...
    //cairo_set_source_rgb(cr, gcolor.red , gcolor.green , gcolor.blue);
}

static void bitmapSetLineType(
	cairo_t *cr,
    double lineWidth,
...
void wlibBitMapDrawLine(
    ...
    wDrawOpts opts)
{
    bitmapSetColor(bd->cr, color);
    bitmapSetLineType(bd->cr, width, lineType, opts);
    ...
    wDrawOpts opts)
{
    ...
    if(!(opts & wDrawOutlineFont)) {
		...
		line = pango_layout_get_line_readonly (layout, 0);
	        bitmapSetLineType( cr, wDrawLineSolid, 0, 0 );
