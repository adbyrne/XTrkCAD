...
wWin_p wMain( int argc, char * argv[] )
{
	...
	mainW = wWinMainCreate( "dtest", 0, 0, "Help", "Main", F_MENUBAR|F_AUTOSIZE,
	                        mainEvent, NULL );
	...
	w2 = wWinPopupCreate( mainW, 2, 2, "Help2", "2 Resize", F_AUTOSIZE|F_RESIZE,
	                      winEvent, (void*)2 );
	d[2] = wDrawCreate( w2, 2, 2, "Draw-1", 0, 200, 200, w2, NULL, doMouse );
	wLineCreate( w2, "", 6, lines );

	w3 = wWinPopupCreate( mainW, 2, 2, "Help3", "3 MenuBar", F_AUTOSIZE|F_MENUBAR,
	                      winEvent, (void*)3 );
	d[3] = wDrawCreate( w3, 2, 2, "Draw-3", 0, 200, 200, w3, NULL, doMouse );
	m3 = wMenuBarAdd( w3, "HelpMB3", "Menu3" );
	wLineCreate( w3, "", 6, lines );

	w4 = wWinPopupCreate( mainW, 2, 2, "Help4", "4 Resize|MenuBar",
	                      F_AUTOSIZE|F_MENUBAR|F_RESIZE, winEvent, (void*)4 );
...
#ifdef LATER
	w5 = wWinPopupCreate( mainW, 3, 3, "Help5", "5 Footer", F_AUTOSIZE|F_FOOTER,
	                      winEvent, (void*)5 );
	d[5] = wDrawCreate( w5, 2, 2, "Draw-5", 0, 200, 200, w5, NULL, NULL );
	wLineCreate( w5, "", 6, lines );

	w6 = wWinPopupCreate( mainW, 3, 3, "Help6", "6 Footer|Resize",
	                      F_AUTOSIZE|F_FOOTER|F_RESIZE, winEvent, (void*)6 );
	d[6] = wDrawCreate( w6, 2, 2, "Draw-6", 0, 200, 200, w6, NULL, NULL );
	wLineCreate( w6, "", 6, lines );

	w7 = wWinPopupCreate( mainW, 3, 3, "Help7", "7 Footer|MenuBar",
	                      F_AUTOSIZE|F_FOOTER|F_MENUBAR, winEvent, (void*)7 );
	d[7] = wDrawCreate( w7, 2, 2, "Draw-7", 0, 200, 200, w7, NULL, NULL );
	m7 = wMenuBarAdd( w7, "HelpMB7", "Menu7" );
	wLineCreate( w7, "", 6, lines );

	w8 = wWinPopupCreate( mainW, 3, 3, "Help8", "8 Footer|Resize|MenuBar",
	                      F_AUTOSIZE|F_FOOTER|F_MENUBAR|F_RESIZE, winEvent, (void*)8 );
