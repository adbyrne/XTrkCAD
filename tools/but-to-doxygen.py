...
def write_dox(pages, source_file, out_dir, keep_template=False):
	...
	# into addm.but's "Command Menus" chapter). Every file, including
	# intro.but.in, writes only ordinary \page blocks now -- the top-level
	# \mainpage table of contents is a separate synthetic page (see
	# write_mainpage(), called once from main() after every real file is
	# written).
...
def write_mainpage(out_dir, title, subpages):
	"""Synthetic top-level table-of-contents page -- \\mainpage's own
	content, separate from intro.but.in's own "index" page. Matches
	halibut's real contents.html (confirmed via real user review,
	2026-08-09): Chapter 1 Introduction To XTrackCAD, Chapter 2 Command
	Menus, Chapter 3 Navigation, Appendix A-E, Index -- nine siblings at
	the top level.

	The earlier design promoted intro.but.in's own \\C{index} chapter
	directly to \\mainpage, which incorrectly flattened "Introduction"'s
	own 11 \\H{} children (Why Choose XTrackCAD, Key XTrackCAD Features,
	etc.) into siblings of Command Menus/Navigation/the appendices instead
	of nesting them under "Introduction" the way halibut does -- the same
	class of cross-file-vs-nested-heading bug as the Command Menus fix
	elsewhere in this file, just within one file instead of across files.

	\\mainpage's own anchor doesn't need to be "index" -- confirmed via an
	isolated Doxygen build that Doxygen maps whichever page has \\mainpage
	to index.html/the "Main Page" tab regardless of its own anchor text,
	so this page needs no anchor of its own at all (nothing \\refs it)."""
	out_path = os.path.join(out_dir, 'toc.dox')
	with open(out_path, 'w', encoding='utf-8') as f:
		f.write('/* Synthetic top-level table of contents, generated by\n')
		f.write(' * tools/but-to-doxygen.py -- see write_mainpage(). Matches\n')
		f.write(" * halibut's real contents.html top-level chapter/appendix list. */\n\n")
		f.write('/**\n')
		f.write(f'\\mainpage {title}\n\n')
		for anchor in subpages:
			f.write(f'- \\subpage {anchor}\n')
		f.write('\n*/\n')
	return out_path
...
def main():
	...
	# intro.but.in's own top chapter keeps halibut's own anchor id "index"
	# (\C{index} Introduction To XTrackCAD) -- but "index" is Doxygen's own
	# RESERVED implicit \ref target for whichever page has \mainpage,
	# regardless of that page's own anchor text (confirmed via an isolated
	# Doxygen build), and write_mainpage() below writes an anchor-less
	# \mainpage of its own. Since this chapter is no longer \mainpage
	# itself (see write_mainpage()'s docstring), keeping its anchor
	# literally "index" collides ("multiple use of section label 'index'
	# for main page", confirmed via a real build, 2026-08-09). Nothing
	# else in the corpus references this anchor via \K{index}/\k{index}
	# (confirmed via a full-corpus grep), so renaming it here, once, right
	# after parsing and before any linking, is safe -- every downstream
	# reference (children's .parent, this page's own .children, the
	# top-level list below) reads the renamed value consistently.
	for p in pages_by_file['intro.but.in']:
		if p.anchor == 'index':
			p.anchor = 'introduction'
		# Every one of this chapter's own \H{} children recorded "index" as
		# their OWN .parent during parsing (the stack held that string
		# before the rename above) -- rename here too, or the children-
		# linking loop below silently fails to find "index" in
		# by_anchor_global (which is keyed by the NEW anchor) and this
		# whole chapter's 11 real children (Why Choose XTrackCAD, Key
		# Features, etc.) end up with no \subpage list at all (confirmed
		# real via a full rebuild, 2026-08-09).
		if p.parent == 'index':
			p.parent = 'introduction'
	...
	# becomes the synthetic \mainpage's \subpage list (see
	# write_mainpage()), so the whole corpus is one ordered tree instead
	# of a pile of alphabetically-sorted unparented roots (see
	# CANONICAL_FILE_ORDER's comment). intro.but.in's own root ("index",
	# "Introduction To XTrackCAD") is INCLUDED here now, as the first
	# top-level sibling -- unlike the earlier design (see
	# write_mainpage()'s docstring for why that was wrong), it is no
	# longer itself promoted to \mainpage and no longer swallows the other
	# chapters as its own children.
	top_level_chapters = []
	for fname in but_files:
		top_level_chapters += [p.anchor for p in pages_by_file[fname] if p.parent is None]
	# halibut's real contents.html ends with a trailing "Index" sibling,
	# after Warranty/License/Copying -- write_index_page() below always
	# produces this anchor.
	top_level_chapters.append('index-generated')

	# messages.but's real book position: NOT a 10th top-level sibling --
	# appendix.but's own \A{messages} heading ("Messages and Explanations",
	# with its own \H{messageType} "Message Types" child) IS halibut's
	# real Appendix C, already correctly picked up above as one of
	# appendix.but's own top-level roots. The ~130 individual message
	# pages this generates are a second child of THAT appendix, appended
	# after "Message Types" (matching real book order: messages.but comes
	# right after appendix.but in app/doc/CMakeLists.txt) -- confirmed via
	# a real user review of halibut's own contents.html showing only ONE
	# "Appendix C: Messages and Explanations" entry, not two (2026-08-09).
	# Spliced in unconditionally (not gated on the generated messages.dox
	# actually existing on disk): app/doc-doxygen-poc/CMakeLists.txt's
	# help-doxygen-full target always regenerates it before building, so
	# by the time Doxygen actually runs the \subpage reference here
	# resolves; if that generation step were ever skipped, Doxygen's own
	# "unable to resolve reference" warning is the right signal, not a
	# silent gap here.
	if 'messages' in by_anchor_global:
		by_anchor_global['messages'].children.append(MESSAGES_ROOT_ANCHOR)
	...
	mainpage_path = write_mainpage(out_dir, "XTrackCAD User's Manual", top_level_chapters)
	...
	print(f'Main page (table of contents): {mainpage_path}')
