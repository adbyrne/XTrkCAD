/** @file drawprim.c
 * @brief Low-level drawing primitives, text rendering, and draw-command tables.
 *
 * Contains coordinate conversion, the DDraw* screen-rendering functions,
 * the TempSeg* segment-capture functions, the draw-command function tables,
 * and the main/temp drawing contexts (mainD, tempD).
 */
...
#include "drawruler.h"   // @brief for lborder, bborder

/** @brief Maximum font size passed to the underlying wlib text functions. */
static wFontSize_t drawMaxTextFontSize = 100;

/** @brief Maximum straight-line segment length (pixels) for arc tessellation. */
static long maxArcSegStraightLen = 100;
...
/**
 * @brief Convert a world coordinate to a pixel position on the main canvas.
 *
 * Applies the current pan origin, scale, and DPI.  Adds the ruler border
 * offsets (lborder, bborder) so that pixel (0,0) is the top-left corner of
 * the border strip rather than the drawable area.
 *
 * @param d   Drawing context.
 * @param p   World coordinate to convert.
 * @param x   OUT pixel X.
 * @param y   OUT pixel Y.
 */
...
/** @brief When TRUE, snap coordinates to the next sub-pixel bin. */
static int Pix2CoOrd_interpolate = 0;

/**
 * @brief Convert a pixel position on the main canvas to a world coordinate.
 *
 * Subtracts the ruler border offsets, then applies DPI and scale.
 *
 * @param d    Drawing context.
 * @param px   Pixel X.
 * @param py   Pixel Y.
 * @param posR OUT world coordinate.
 */
...
/** @brief Build the wDrawOpts flags from a drawing context's option bits. */
#define DRAWOPTS(D)                                                            \
  (((D->options & DC_TEMP) ? wDrawOptTemp : 0) |                               \
   ((D->options & DC_OUTLINE) ? wDrawOutlineFont : 0))

/**
 * @brief Draw a line segment on the screen.
 */
...
/**
 * @brief Draw an arc (or approximate with line segments if radius is too large).
 */
 ...
 *
 * When the color is white a filled rectangle is drawn instead to erase
...
/**
 * @brief Draw a polygon on the screen.
 */
static void DDrawPoly(drawCmd_p d, int cnt, coOrd *pts, int *types,
                      wDrawColor color, wDrawWidth width, drawFill_e eFillOpt) {
...
/**
 * @brief Draw a filled circle on the screen, with border-clipping guard.
 */
static void DDrawFillCircle(drawCmd_p d, coOrd p, DIST_T r, wDrawColor color) {
  ...
  rr = (r / d->scale) * d->dpi + 0.5;
  if (rr > wDrawGetMaxRadius(d->d)) {
    // @brief Circle radius exceeds maximum renderable size
    return;
  }
  d->CoOrd2Pix(d, p, &x, &y);
  wWinPix_t w, h;
...
/**
 * @brief Draw a filled or outlined rectangle on the screen.
 */
static void DDrawRectangle(drawCmd_p d, coOrd orig, coOrd size,
  ...
  if (drawEnable) {
    ...
    switch (eFillOpt) {
    case DRAW_CLOSED:
      // @brief Draw four sides: bottom-left origin, counter-clockwise
      // 1 2
      // 0 3
      p1.x = orig.x;
      p1.y = orig.y + size.y;
...
/**
 * @brief Draw a bitmap glyph at a world coordinate.
 */
...
/**
 * @brief Draw text inside an optionally framed/filled box.
 *
 * The @p style parameter controls whether the box is framed, underlined,
 * inverted, or drawn with a white background.  The @p pos argument is
 * interpreted according to the BOX_POS_* flags in @p style.
 *
 * @param style   BOX_* style flags (see draw.h).
 * @param d       Drawing context.
 * @param pos     Anchor point — meaning depends on BOX_POS_* flags.
 * @param text    Null-terminated string to draw.
 * @param fp      Font to use.
 * @param fs      Font size in world units (points * scale).
 * @param color   Text and border colour.
 * @param a       Rotation angle (currently unused for rulers).
 */
EXPORT void DrawBoxedString(int style, drawCmd_p d, coOrd pos, char *text,
  ...
  if (style & BOX_POS_TOP_CENTER) {
    // @brief Centre horizontally; p0.y is already the top edge
    p0.x -= size.x / 2.0;
  }
  style &= ~BOX_POS_TOP_CENTER;

  if (style & BOX_POS_LEFT_CENTER) {
    // @brief p0 is left edge, vertically centre on it
    p0.y -= size.y / 2.0;
  }
  ...
  if ((style & BOX_POS_BOTTOM_LEFT) == 0) {
    if ((style & BOX_POS_BOTTOM_RIGHT) != 0) {
      // @brief p0 is bottom-right; shift left by full width to get bottom-left
      p0.x -= size.x - 1;
      p0.y -= size.y / 2.0 - 1.5;
    } else {
      // @brief p0 is centre; shift to bottom-left
      p0.x -= size.x / 2.0;
      p0.y -= size.y / 2.0;
    }
  }
...
/**
 * @brief Compute the rendered size of a text string in world coordinates.
 *
 * @param dp        Drawing context (used for scale and DPI).
 * @param text      String to measure.
 * @param fp        Font.
 * @param fs        Font size.
 * @param relative  TRUE to return size in world units; FALSE in pixels.
 * @param size      OUT bounding box.
 * @param descent   OUT distance below baseline.
 * @param ascent    OUT distance above baseline.
 */
...
/**
 * @brief Convenience wrapper that discards descent and ascent.
 */
...
/**
 * @brief Compute the bounding box of multi-line text.
 *
 * @param dp        Drawing context.
 * @param text      Newline-separated string.
 * @param fp        Font.
 * @param fs        Font size.
 * @param relative  TRUE to return world units.
 * @param size      OUT total bounding box.
 * @param lastline  OUT width and relative Y of the last line.
 */
...
/*****************************************************************************
 *
 * TEMPORARY SEGMENT CAPTURE FUNCTIONS
 *
 */

/** @brief Capture a line segment into the tempSegs dynamic array. */
...
/** @brief Capture an arc segment into the tempSegs dynamic array. */
...
/** @brief Capture a text segment into the tempSegs dynamic array. */
...
/** @brief Capture a polygon segment into the tempSegs dynamic array. */
...
/** @brief Capture a filled circle into the tempSegs dynamic array. */
...
/** @brief Capture a rectangle into the tempSegs dynamic array as a polygon. */
static void TempSegRectangle(drawCmd_p d, coOrd orig, coOrd size,
                             wDrawColor color, drawFill_e eOpts) {
  coOrd p[4];
  // @brief Corner layout: p1-p2 / p0-p3 (bottom-left origin)
  p[0].x = p[1].x = orig.x;
...
/** @brief No-op bitmap draw function used by non-screen draw contexts. */
static void NoDrawBitMap(drawCmd_p d, coOrd p, wDrawBitMap_p bm,
                         wDrawColor color) {}

/*****************************************************************************
 *
 * DRAW COMMAND FUNCTION TABLES AND CONTEXTS
 *
 */
...
EXPORT drawCmd_t mainD = {
    ...
    {0.0, 0.0}, {0.0, 0.0},       MainPix2CoOrd, MainCoOrd2Pix,   96.0};
